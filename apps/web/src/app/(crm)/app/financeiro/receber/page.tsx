...
import { createReceivable, listEntries } from "../actions";
...
export default async function ReceivablesPage() {
  const rows = await listEntries({ scope: "company", direction: "in" });

  return (
    <>
      <PageHeader eyebrow="Financeiro" title="Contas a receber" description="Parcelas, baixas manuais e estornos. Regime de caixa, sem cobrança automática." />

      <details className="mb-5 rounded-2xl border border-[var(--line)] bg-[var(--surface)] p-5">
        <summary className="cursor-pointer text-sm font-extrabold">+ Novo lançamento manual</summary>
        <div className="mt-4"><NewEntryForm action={createReceivable} submitLabel="Criar lançamento" /></div>
      </details>
      ...
      <Card className="overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full min-w-[820px] text-left">
            ...
            <tbody>
              {rows.length === 0 && <tr><td colSpan={6} className="px-5 py-10 text-center text-sm text-[var(--muted)]">Nenhum lançamento ainda.</td></tr>}
              {rows.map((entry) => (
