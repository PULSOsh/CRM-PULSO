import { Bot, CalendarDays, CreditCard, Mail, MessageCircle, PenTool } from "lucide-react";
...
const integrations = [
  ...
  { icon: MessageCircle, title: "Telegram", description: "Alertas administrativos privados, sem ações financeiras.", status: "Opcional" as const },
...
export default function IntegrationsPage() {
  return (
    <>
      ...
      <div className="mb-5 rounded-2xl border border-[var(--line)] bg-[var(--soft)] p-4 text-sm leading-6 text-[var(--muted-strong)]"><strong className="text-[var(--carbon)]">Modo seguro:</strong> todos os módulos possuem alternativa manual. Erros de conexão geram aviso, mas não travam o sistema.</div>
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{integrations.map(i=><IntegrationCard key={i.title} {...i} />)}</div>
    </>
  );
}
