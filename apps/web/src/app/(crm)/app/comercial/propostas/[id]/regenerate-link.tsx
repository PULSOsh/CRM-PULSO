...
import { regenerateProposalLink } from "../actions";

export function RegenerateProposalLink({ proposalId }: { proposalId: string }) {
  const [link, setLink] = useState<string | null>(null);
  const [pending, startTransition] = useTransition();

  function handleClick() {
    startTransition(async () => {
      const result = await regenerateProposalLink(proposalId);
