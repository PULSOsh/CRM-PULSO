...
import { ProcessDueButton, RecurrenceRowActions } from "./recurrence-actions";
...
export default async function RecurrencesPage() {
  const rows = await db.select()
    ...
    .where(eq(schema.financialRecurrences.scope, "company"))
      ...
      <Card className="overflow-hidden bg-[var(--surface)]/80 border border-[var(--line)] shadow-xl">
        <div className="overflow-x-auto">
          <table className="w-full min-w-[900px] text-left">
            ...
            <tbody>
              {rows.length === 0 && <tr><td colSpan={7} className="px-5 py-10 text-center text-sm text-[var(--muted)]">Nenhuma recorrência ainda.</td></tr>}
              {rows.map((entry) => (
                <tr key={entry.id} className="border-b border-[var(--line)] last:border-0 hover:bg-[var(--soft)]/30 transition-colors">
                  ...
                  <td className="px-5 py-4 text-right">
                    <RecurrenceRowActions id={entry.id} status={entry.status} />
                  </td>
                </tr>
              ))}
            </tbody>
